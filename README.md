...
- **`backend/`** — Node.js + Express + SQLite bawaan Node.js (`node:sqlite`) melalui adapter ringan di `backend/db/sqlite-adapter.js`
...
│   │   └── sqlite-adapter.js  # adapter node:sqlite dengan API mirip better-sqlite3
...
└── frontend/
    └── src/
        ...
        ├── lib/            # api.js, ThemeContext, ToastContext, subjectColors
        └── App.jsx / main.jsx
```

## Perbaikan & Peningkatan UI/UX

- **Database**: beralih ke SQLite bawaan Node.js (`node:sqlite`) sehingga tidak butuh build tools C++ untuk menjalankan project.
- **Backend**: middleware error handling terpusat, validasi input (status tugas, jawaban kuis), pengecekan 404, dan transaksi saat menyimpan hasil AI.
- **Frontend**:
  - Dark mode persist ke `localStorage` dengan dukungan preferensi sistem.
  - Layout responsif dengan sidebar mobile.
  - Toast notifikasi untuk feedback (sukses/gagal/info).
  - Search modal (`Ctrl`/`Cmd` + `K`) untuk navigasi cepat.
  - Skeleton loading di dashboard dan loading state di tombol.
  - Halaman Kuis mendukung URL `/kuis/:id`, skor akhir, dan mencegah jawaban ganda.
  - Upload catatan langsung mengarah ke kuis yang baru dibuat.
  - Halaman Tugas bisa hapus tugas dan toggle status dengan feedback toast.
  - Halaman 404, empty state, dan animasi transisi halus.
