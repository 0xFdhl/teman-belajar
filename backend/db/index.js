...
import { DatabaseAdapter } from "./sqlite-adapter.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const dbPath = path.join(__dirname, "teman-belajar.sqlite3");

export const db = new DatabaseAdapter(dbPath);
db.pragma("foreign_keys = ON");

// Migrasi: tambah kolom error_message jika belum ada
try {
  db.exec("ALTER TABLE notes ADD COLUMN error_message TEXT");
} catch {
...
export function seedIfEmpty() {
  ...
  // Gunakan tanggal relatif terhadap hari ini supaya tidak langsung overdue
