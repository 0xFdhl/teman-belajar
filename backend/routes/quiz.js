...
router.post("/questions/:questionId/answer", (req, res) => {
  ...
  const upperSelected = selected_option?.toString().trim().toUpperCase();

  if (!["A", "B", "C", "D"].includes(upperSelected)) {
    return res.status(400).json({ error: "Jawaban harus A, B, C, atau D." });
  }
  ...
  const isCorrect = upperSelected === question.correct_option ? 1 : 0;

  db.prepare(
    `INSERT INTO attempts (question_id, selected_option, is_correct) VALUES (?, ?, ?)`
  ).run(question.id, upperSelected, isCorrect);
